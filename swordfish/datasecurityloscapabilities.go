...
func ListReferencedDataSecurityLoSCapabilities(c common.Client, link string) ([]*DataSecurityLoSCapabilities, error) { //nolint:dupl
	...
	// Save unordered results into link-to-DataSecurityLoSCapabilities helper map.
	unorderedResults := map[string]*DataSecurityLoSCapabilities{}
	for r := range ch {
		if r.Error != nil {
	...
	results := make([]*DataSecurityLoSCapabilities, len(links))

...
func ListReferencedIOPerformanceLoSCapabilitiess(c common.Client, link string) ([]*IOPerformanceLoSCapabilities, error) { //nolint:dupl
	...
	// Save unordered results into link-to-IOPerformanceLoSCapabilities helper map.
	unorderedResults := map[string]*IOPerformanceLoSCapabilities{}
	for r := range ch {
		if r.Error != nil {
	...
	results := make([]*IOPerformanceLoSCapabilities, len(links))

...
func ListReferencedDataProtectionLineOfServices(c common.Client, link string) ([]*DataProtectionLineOfService, error) { //nolint:dupl
	...
	// Save unordered results into link-to-DataProtectionLineOfService helper map.
	unorderedResults := map[string]*DataProtectionLineOfService{}
	for r := range ch {
		if r.Error != nil {
	...
	results := make([]*DataProtectionLineOfService, len(links))

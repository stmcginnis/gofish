...
func (f *FileSystem) UnmarshalJSON(b []byte) error {
	...
	type fLinks struct {
		...
		ReplicaCollection common.Links `json:"ReplicaCollection"`
		SpareResourceSets common.Links `json:"SpareResourceSets"`
	}
	var tmp struct {
		temp
		Links          fLinks
		ExportedShares common.Links `json:"exportedShares"`
		ImportedShares common.Links `json:"ImportedShares"`
	...
	*f = FileSystem(tmp.temp)
	...
	f.exportedShares = tmp.ExportedShares.ToStrings()
	f.importedShares = tmp.ImportedShares.ToStrings()
...
func (f *FileSystem) ClassOfService(client common.Client) (*ClassOfService, error) {
	if f.classOfService == "" {
		return nil, nil
	}
...
func (f *FileSystem) ReplicaCollection(client common.Client) ([]*FileSystem, error) {
	return common.GetObjects[FileSystem](client, f.replicaCollection)
}

// SpareResourceSets gets the SpareResourceSets linked resources.
func (f *FileSystem) SpareResourceSets(client common.Client) ([]*SpareResourceSet, error) {
	return common.GetObjects[SpareResourceSet](client, f.spareResourceSets)
}

// ExportedShares gets the ExportedShares collection.
func (f *FileSystem) ExportedShares(client common.Client) ([]*FileShare, error) {
	return common.GetObjects[FileShare](client, f.exportedShares)
}

// ImportedShares gets the ImporteddShares collection.
func (f *FileSystem) ImportedShares(client common.Client) ([]*FileShare, error) {
	return common.GetObjects[FileShare](client, f.importedShares)
}

// Metrics gets the Metrics linked resource.
func (f *FileSystem) Metrics(client common.Client) (*FileSystemMetrics, error) {
	if f.metrics == "" {
		return nil, nil
	}
	return common.GetObject[FileSystemMetrics](client, f.metrics)
}

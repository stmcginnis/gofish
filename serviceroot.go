...
//
// 2025.2 - #Service.v1_19_0.Service
...
type Service struct {
	...
	// shall not be present if its value is an empty string or 'null'.
	//
...
func (s *Service) UnmarshalJSON(b []byte) error {
	...
	var tmp struct {
		temp
		Links              sLinks
		AccountService     common.Link `json:"accountService"`
		AggregationService common.Link `json:"aggregationService"`
		AutomationNodes    common.Link `json:"automationNodes"`
		Cables             common.Link `json:"cables"`
		CertificateService common.Link `json:"certificateService"`
		Chassis            common.Link `json:"chassis"`
		ComponentIntegrity common.Link `json:"componentIntegrity"`
		CompositionService common.Link `json:"compositionService"`
		EventService       common.Link `json:"eventService"`
		Fabrics            common.Link `json:"fabrics"`
		Facilities         common.Link `json:"facilities"`
		JobService         common.Link `json:"jobService"`
		JSONSchemas        common.Link `json:"jsonSchemas"`
		KeyService         common.Link `json:"keyService"`
		LicenseService     common.Link `json:"licenseService"`
		Managers           common.Link `json:"managers"`
		NVMeDomains        common.Link `json:"nVMeDomains"`
		PowerEquipment     common.Link `json:"powerEquipment"`
		RegisteredClients  common.Link `json:"registeredClients"`
		Registries         common.Link `json:"registries"`
		ResourceBlocks     common.Link `json:"resourceBlocks"`
		ServiceConditions  common.Link `json:"serviceConditions"`
		SessionService     common.Link `json:"sessionService"`
		Storage            common.Link `json:"storage"`
		StorageServices    common.Link `json:"storageServices"`
		StorageSystems     common.Link `json:"storageSystems"`
		Systems            common.Link `json:"systems"`
		Tasks              common.Link `json:"tasks"`
		TelemetryService   common.Link `json:"telemetryService"`
		ThermalEquipment   common.Link `json:"thermalEquipment"`
		UpdateService      common.Link `json:"updateService"`
	}
...
func (s *Service) CreateSession(username, password string) (*redfish.AuthToken, error) {
	return redfish.CreateSession(s.GetClient(), s.sessions, username, password)
}

// DeleteSession logout the specified session
func (s *Service) DeleteSession(url string) error {
	return redfish.DeleteSession(s.GetClient(), url)
}

// ManagerProvidingService gets the ManagerProvidingService linked resource.
func (s *Service) ManagerProvidingService() (*redfish.Manager, error) {
	if s.managerProvidingService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.Manager](s.GetClient(), s.managerProvidingService)
}
...
func (s *Service) Sessions() (*redfish.Session, error) {
	...
	return common.GetObject[redfish.Session](s.GetClient(), s.sessions)
}
...
func (s *Service) AccountService() (*redfish.AccountService, error) {
	...
	return common.GetObject[redfish.AccountService](s.GetClient(), s.accountService)
}

// AggregationService gets the AggregationService linked resource.
func (s *Service) AggregationService() (*redfish.AggregationService, error) {
	if s.aggregationService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.AggregationService](s.GetClient(), s.aggregationService)
}

// AutomationNodes gets the AutomationNodes collection.
func (s *Service) AutomationNodes() ([]*redfish.AutomationNode, error) {
	if s.automationNodes == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.AutomationNode](s.GetClient(), s.automationNodes)
}

// Cables gets the Cables collection.
func (s *Service) Cables() ([]*redfish.Cable, error) {
	if s.cables == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.Cable](s.GetClient(), s.cables)
}

// CertificateService gets the CertificateService linked resource.
func (s *Service) CertificateService() (*redfish.CertificateService, error) {
	if s.certificateService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.CertificateService](s.GetClient(), s.certificateService)
}
...
func (s *Service) Chassis() ([]*redfish.Chassis, error) {
	...
	return common.GetCollectionObjects[redfish.Chassis](s.GetClient(), s.chassis)
}

// ComponentIntegrity gets the ComponentIntegrity collection.
func (s *Service) ComponentIntegrity() ([]*redfish.ComponentIntegrity, error) {
	if s.componentIntegrity == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.ComponentIntegrity](s.GetClient(), s.componentIntegrity)
}
...
func (s *Service) CompositionService() (*redfish.CompositionService, error) {
	...
	return common.GetObject[redfish.CompositionService](s.GetClient(), s.compositionService)
}
...
func (s *Service) EventService() (*redfish.EventService, error) {
	...
	return common.GetObject[redfish.EventService](s.GetClient(), s.eventService)
}

// Fabrics gets the Fabrics collection.
func (s *Service) Fabrics() ([]*redfish.Fabric, error) {
	if s.fabrics == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.Fabric](s.GetClient(), s.fabrics)
}

// Facilities gets the Facilities collection.
func (s *Service) Facilities() ([]*redfish.Facility, error) {
	if s.facilities == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.Facility](s.GetClient(), s.facilities)
}

// JobService gets the JobService linked resource.
func (s *Service) JobService() (*redfish.JobService, error) {
	if s.jobService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.JobService](s.GetClient(), s.jobService)
}

// JsonSchemas gets the JsonSchemas collection.
func (s *Service) JSONSchemas() ([]*redfish.JSONSchemaFile, error) {
	if s.jsonSchemas == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.JSONSchemaFile](s.GetClient(), s.jsonSchemas)
}

// KeyService gets the KeyService linked resource.
func (s *Service) KeyService() (*redfish.KeyService, error) {
	if s.keyService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.KeyService](s.GetClient(), s.keyService)
}

// LicenseService gets the LicenseService linked resource.
func (s *Service) LicenseService() (*redfish.LicenseService, error) {
	if s.licenseService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.LicenseService](s.GetClient(), s.licenseService)
}
...
func (s *Service) Managers() ([]*redfish.Manager, error) {
	...
	return common.GetCollectionObjects[redfish.Manager](s.GetClient(), s.managers)
}

// NVMeDomains gets the NVMeDomains collection.
func (s *Service) NVMeDomains() ([]*swordfish.NVMeDomain, error) {
	if s.nVMeDomains == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[swordfish.NVMeDomain](s.GetClient(), s.nVMeDomains)
}

// PowerEquipment gets the PowerEquipment linked resource.
func (s *Service) PowerEquipment() (*redfish.PowerEquipment, error) {
	if s.powerEquipment == "" {
		return nil, nil
	}
	return common.GetObject[redfish.PowerEquipment](s.GetClient(), s.powerEquipment)
}

// RegisteredClients gets the RegisteredClients collection.
func (s *Service) RegisteredClients() ([]*redfish.RegisteredClient, error) {
	if s.registeredClients == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.RegisteredClient](s.GetClient(), s.registeredClients)
}

// Registries gets the Registries collection.
func (s *Service) Registries() ([]*redfish.MessageRegistryFile, error) {
	if s.registries == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.MessageRegistryFile](s.GetClient(), s.registries)
}

// ResourceBlocks gets the ResourceBlocks collection.
func (s *Service) ResourceBlocks() ([]*redfish.ResourceBlock, error) {
	if s.resourceBlocks == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.ResourceBlock](s.GetClient(), s.resourceBlocks)
}

// ServiceConditions gets the ServiceConditions linked resource.
func (s *Service) ServiceConditions() (*redfish.ServiceConditions, error) {
	if s.serviceConditions == "" {
		return nil, nil
	}
	return common.GetObject[redfish.ServiceConditions](s.GetClient(), s.serviceConditions)
}

// SessionService gets the SessionService linked resource.
func (s *Service) SessionService() (*redfish.SessionService, error) {
	if s.sessionService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.SessionService](s.GetClient(), s.sessionService)
}

// Storage gets the Storage collection.
func (s *Service) Storage() ([]*redfish.Storage, error) {
	if s.storage == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[redfish.Storage](s.GetClient(), s.storage)
}
...
func (s *Service) StorageServices() ([]*swordfish.StorageService, error) {
	...
	return common.GetCollectionObjects[swordfish.StorageService](s.GetClient(), s.storageServices)
}
...
func (s *Service) StorageSystems() ([]*swordfish.StorageSystem, error) {
	...
	return common.GetCollectionObjects[swordfish.StorageSystem](s.GetClient(), s.storageSystems)
}
...
func (s *Service) Systems() ([]*redfish.ComputerSystem, error) {
	...
	return common.GetCollectionObjects[redfish.ComputerSystem](s.GetClient(), s.systems)
}

// Tasks gets the Tasks linked resource.
func (s *Service) Tasks() (*redfish.TaskService, error) {
	if s.tasks == "" {
		return nil, nil
	}
	return common.GetObject[redfish.TaskService](s.GetClient(), s.tasks)
}

// TelemetryService gets the TelemetryService linked resource.
func (s *Service) TelemetryService() (*redfish.TelemetryService, error) {
	if s.telemetryService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.TelemetryService](s.GetClient(), s.telemetryService)
}

// ThermalEquipment gets the ThermalEquipment linked resource.
func (s *Service) ThermalEquipment() (*redfish.ThermalEquipment, error) {
	if s.thermalEquipment == "" {
		return nil, nil
	}
	return common.GetObject[redfish.ThermalEquipment](s.GetClient(), s.thermalEquipment)
}

// UpdateService gets the UpdateService linked resource.
func (s *Service) UpdateService() (*redfish.UpdateService, error) {
	if s.updateService == "" {
		return nil, nil
	}
	return common.GetObject[redfish.UpdateService](s.GetClient(), s.updateService)
}

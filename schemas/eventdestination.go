...
// http://redfish.dmtf.org/schemas/v1/EventDestination.v1_16_0.json
...
func (e *EventDestination) ResumeSubscription(deliverBufferedEventDuration string) (*TaskMonitorInfo, error) {
	...
	resp, taskInfo, err := PostWithTask(e.client,
		e.resumeSubscriptionTarget, payload, e.ActionHeaders(e.resumeSubscriptionTarget), false)
...
func (e *EventDestination) SuspendSubscription() (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	resp, taskInfo, err := PostWithTask(e.client,
		e.suspendSubscriptionTarget, payload, e.ActionHeaders(e.suspendSubscriptionTarget), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

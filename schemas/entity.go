...
// APIFilter represents query filter options for API requests.
type APIFilter string

// APIFilterOption defines functions that can modify APIFilter settings.
type APIFilterOption func(*APIFilter)

// WithSkip adds a $skip parameter to the filter.
func WithSkip(skipNum int) APIFilterOption {
	return func(e *APIFilter) {
		*e = APIFilter(fmt.Sprintf("%s$skip=%d", *e, skipNum))
	}
}

// WithTop adds a $top parameter to the filter.
func WithTop(topNum int) APIFilterOption {
	return func(e *APIFilter) {
		*e = APIFilter(fmt.Sprintf("%s$top=%d", *e, topNum))
	}
}

// SetFilter configures the filter with the provided options.
func (e *APIFilter) SetFilter(opts ...APIFilterOption) {
	*e = "?"
	lastIdx := len(opts) - 1
...
// ClearAPIFilter resets the filter to empty.
func (e *APIFilter) ClearFilter() {
	*e = ""
}

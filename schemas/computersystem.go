...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (c *ComputerSystem) Decommission(computerSystemETag string, decommissionTypes []DecommissionType, oEMDecommissionTypes []string, requireSecureErase bool) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	payload["ComputerSystemETag"] = computerSystemETag
	...
	resp, taskInfo, err := PostWithTask(c.client,
		c.decommissionTarget, payload, c.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}
...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (c *ComputerSystem) ExportConfiguration(components []Component, encryptionPassphrase string, exportType ExportType, oEMComponents []string, security ExportSecurity) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	...
	resp, taskInfo, err := PostWithTask(c.client,
		c.exportConfigurationTarget, payload, c.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

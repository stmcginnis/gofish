...
type CollectDiagnosticDataParameters struct {
	...
	TargetDevice Link `json:",omitempty"`
	...
	TransferProtocol LogServiceTransferProtocolType `json:",omitempty"`
}
...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (l *LogService) PushDiagnosticData(additionalDataURI string, password string, targetURI string, transferProtocol LogServiceTransferProtocolType, userName string) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	...
	resp, taskInfo, err := PostWithTask(l.client,
		l.pushDiagnosticDataTarget, payload, l.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

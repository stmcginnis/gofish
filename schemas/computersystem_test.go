...
func TestComputerSystem(t *testing.T) { //nolint
	...
	if result.Status.HealthRollup != OKHealth {
		t.Errorf("Received invalid health rollup status: %s", result.Status.HealthRollup)
	}

	if result.IndicatorLED != OffIndicatorLED {
	...
	if result.MemorySummary.Status.State != EnabledState {
	...
	if result.TrustedModules[0].Status.Health != OKHealth {
...
func TestComputerSystemUpdate(t *testing.T) {
	...
	result.IndicatorLED = BlinkingIndicatorLED
	err = result.Update()

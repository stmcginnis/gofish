...
func (e *EventService) SubmitFullTestEvent(eventGroupID int, eventID string, eventTimestamp string, eventType EventType, message string, messageArgs string, messageID string, messageSeverity Health, originOfCondition string, severity string) error {
	payload := make(map[string]any)

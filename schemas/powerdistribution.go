...
func (p *PowerDistribution) ExportConfiguration(components []Component, encryptionPassphrase string, exportType ExportType, oEMComponents []string, security ExportSecurity) (*TaskMonitorInfo, error) {
	...
	resp, taskInfo, err := PostWithTask(p.client,
		p.exportConfigurationTarget, payload, p.Headers(), false)

...
// commonName - This parameter shall contain the of the component to secure, as
// defined by the RFC5280 'commonName' attribute.
...
func (c *CertificateService) ReplaceCertificate(certificateString string, certificateType CertificateType, certificateURI string, password string) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	...
	resp, taskInfo, err := PostWithTask(c.client,
		c.replaceCertificateTarget, payload, c.Headers(), false)

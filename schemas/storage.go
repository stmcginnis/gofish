...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (s *Storage) ResetToDefaults(resetType StorageResetToDefaultsType) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
...
// Specification.
// authenticationMethod - This parameter shall contain the authentication
// method for the unfreeze operation as defined by the 'Authenticated Unfreeze
// Support' field in the NVMe Base Specification.
// key - This parameter shall contain the authentication key for the unfreeze
// operation as a hex-encoded string. If 'AuthenticationMethod' contains
// 'ProgrammedKey', this parameter contains a key that is derived from the
// original key provided in the 'SetPersonalityKey' action. If
// 'AuthenticationMethod' contains 'PhysicalId', this parameter contains the
// physical secure ID (PSID) of the drive converted to a hex-encoded string.
// nonce - This parameter shall contain the personality nonce provided by the
...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (s *Storage) UnfreezePersonality(authenticationMethod NMVeUnfreezeAuthMode, key string, nonce string, personality uint) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	...
	payload["Personality"] = personality
	resp, taskInfo, err := PostWithTask(s.client,
		s.unfreezePersonalityTarget, payload, s.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}
...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (s *StorageStorageController) ResetToDefaults(resetType StorageResetToDefaultsType) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	payload["ResetType"] = resetType
	resp, taskInfo, err := PostWithTask(s.client,
		s.resetToDefaultsTarget, payload, s.Headers(), false)
...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (s *StorageStorageController) UnfreezePersonality(authenticationMethod NMVeUnfreezeAuthMode, key string, nonce string, personality uint) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	payload["AuthenticationMethod"] = authenticationMethod
	payload["Key"] = key
	payload["Nonce"] = nonce

...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (c *ConsistencyGroup) CreateReplicaTarget(consistencyGroupName string, replicaType ReplicaType, replicaUpdateMode ReplicaUpdateMode, targetStoragePool string) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	...
	resp, taskInfo, err := PostWithTask(c.client,
		c.createReplicaTargetTarget, payload, c.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

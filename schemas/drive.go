...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (d *Drive) UnfreezePersonality(authenticationMethod NMVeUnfreezeAuthMode, key string, nonce string, personality uint) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	...
	payload["Personality"] = personality
	resp, taskInfo, err := PostWithTask(d.client,
		d.unfreezePersonalityTarget, payload, d.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

...
// http://redfish.dmtf.org/schemas/v1/Circuit.v1_9_0.json
...
func (c *Circuit) BreakerControl(powerState PowerState) (*TaskMonitorInfo, error) {
	...
	resp, taskInfo, err := PostWithTask(c.client,
		c.breakerControlTarget, payload, c.ActionHeaders(c.breakerControlTarget), false)
...
func (c *Circuit) PowerControl(powerState PowerState) (*TaskMonitorInfo, error) {
	...
	payload["PowerState"] = powerState
	resp, taskInfo, err := PostWithTask(c.client,
		c.powerControlTarget, payload, c.ActionHeaders(c.powerControlTarget), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}
...
func (c *Circuit) ResetMetrics() (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	resp, taskInfo, err := PostWithTask(c.client,
		c.resetMetricsTarget, payload, c.ActionHeaders(c.resetMetricsTarget), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

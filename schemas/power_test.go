...
func TestPower(t *testing.T) {
	...
	if result.PowerSupplies[0].PowerSupplyType != ACPowerSupplyType {
		t.Errorf("Invalid PowerSupply type: %s",
			result.PowerSupplies[0].PowerSupplyType)
	}
...
func TestVoltageUnmarshalJSON_OptionalFields(t *testing.T) {
	...
	if voltage.Status.Health != OKHealth {

...
// image - This parameter shall contain the URI of the media to be attached to
// the virtual media. This parameter may specify an absolute URI to remote
// media or a relative URI to media local to the implementation. A service may
// allow a relative URI to reference a 'SoftwareInventory' resource. Services
// may reject this action if the URI is not accessible.
// inserted - This parameter shall contain whether the image is treated as
// mounted and visible to the host system upon completion of the action. If the
// client does not provide this parameter, the service shall default this value
// to 'true'.
// password - This parameter shall contain the password to access the URI
// specified by the 'Image' parameter.
// transferMethod - This parameter shall contain the transfer method to use
// with the specified image URI.
// transferProtocolType - This parameter shall contain the network protocol to
// use with the URI specified by the 'Image' parameter.
// userName - This parameter shall contain the username to access the URI
// specified by the 'Image' parameter.
// writeProtected - This parameter shall contain whether the remote media is
// treated as write-protected. If the client does not provide this parameter,
// the service shall default this value to 'true'.
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (v *VirtualMedia) InsertMedia(image string, inserted bool, writeProtected bool) (*TaskMonitorInfo, error) {
...
func (v *VirtualMedia) InsertMediaConfig(config VirtualMediaConfig) (*TaskMonitorInfo, error) {
	if !v.SupportsMediaInsert {
		return nil, errors.New("redfish service does not support VirtualMedia.InsertMedia calls")
	}

	resp, taskInfo, err := PostWithTask(v.client,
		v.insertMedia.Target, config, v.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

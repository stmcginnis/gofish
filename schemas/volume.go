...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (v *Volume) ChangeRAIDLayout(drives []string, mediaSpanCount int, rAIDType RAIDType, stripSizeBytes int) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	...
	resp, taskInfo, err := PostWithTask(v.client,
		v.changeRAIDLayoutTarget, payload, v.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}
...
//
// If TaskMonitorInfo is not nil it can be used to monitor async tasks.
func (v *Volume) CreateReplicaTarget(replicaType ReplicaType, replicaUpdateMode ReplicaUpdateMode, targetStoragePool string, volumeName string) (*TaskMonitorInfo, error) {
	payload := make(map[string]any)
	payload["ReplicaType"] = replicaType
	payload["ReplicaUpdateMode"] = replicaUpdateMode
	...
	resp, taskInfo, err := PostWithTask(v.client,
		v.createReplicaTargetTarget, payload, v.Headers(), false)
	defer DeferredCleanupHTTPResponse(resp)
	return taskInfo, err
}

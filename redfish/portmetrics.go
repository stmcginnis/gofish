...
//
// 2025.3 - #PortMetrics.v1_8_0.PortMetrics
...
func (p *PortMetrics) UnmarshalJSON(b []byte) error {
	...
	type pActions struct {
		ResetMetrics common.ActionTarget `json:"#PortMetrics.ResetMetrics"`
	}
	var tmp struct {
		temp
		Actions pActions
...
func (p *PortMetrics) Update() error {
...
// device.
func (p *PortMetrics) ResetMetrics() error {
...
// FibreChannelPortMetric shall describe Fibre Channel-specific metrics for network ports.
type FibreChannelPortMetric struct {
...
// PortMetricsGenZ shall describe the Gen-Z related port metrics.
type PortMetricsGenZ struct {
...
type Transceiver struct {
	...
	// RXInputPowerMilliWatts shall contain the RX input power value of a small
	// form-factor pluggable (SFP) transceiver, aggregated across all lanes of this
	// port.
	//
	// Version added: v1.1.0
	RXInputPowerMilliWatts *float64 `json:",omitempty"`
	// SupplyVoltage shall contain the supply voltage of a small form-factor
	// pluggable (SFP) transceiver.
	//
	// Version added: v1.1.0
	SupplyVoltage *float64 `json:",omitempty"`
	// TXBiasCurrentMilliAmps shall contain the TX bias current value of a small
	// form-factor pluggable (SFP) transceiver, aggregated across all lanes of this
	// port.
	//
	// Version added: v1.1.0
	TXBiasCurrentMilliAmps *float64 `json:",omitempty"`
	// TXOutputPowerMilliWatts shall contain the TX output power value of a small
	// form-factor pluggable (SFP) transceiver, aggregated across all lanes of this
	// port.
	//
	// Version added: v1.1.0
	...
	// a small form-factor pluggable (SFP) transceiver. This property shall not be
	// present for non-optic SFP mediums.
	//
...
type TransceiverLaneMetrics struct {
	...
	// RXInputPowerMilliWatts shall contain the RX input power value of a small
	// form-factor pluggable (SFP) transceiver, specific to the lane identified as
	// 'LaneId'.
	//
	// Version added: v1.8.0
	RXInputPowerMilliWatts *float64 `json:",omitempty"`

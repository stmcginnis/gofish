...
//
// 2025.2 - #ManagerAccount.v1_14_0.ManagerAccount
...
type ManagerAccount struct {
	...
	// expires. If provided during account creation or password modification, this
	// value shall override the value of the 'PasswordExpirationDays' property in
	// the 'AccountService' resource.
	//
...
func (m *ManagerAccount) UnmarshalJSON(b []byte) error {
	...
	type mActions struct {
		...
		ClearSecretKey                 common.ActionTarget `json:"#ManagerAccount.ClearSecretKey"`
		GenerateSecretKey              common.ActionTarget `json:"#ManagerAccount.GenerateSecretKey"`
		VerifyTimeBasedOneTimePassword common.ActionTarget `json:"#ManagerAccount.VerifyTimeBasedOneTimePassword"`
	}
	type mLinks struct {
		Role common.Link `json:"Role"`
	}
	var tmp struct {
		temp
		Actions      mActions
...
// ChangePassword shall change the account password while requiring password for
...
// header.
func (m *ManagerAccount) ChangePassword(newPassword string, sessionAccountPassword string) error {
	payload := make(map[string]any)
	payload["NewPassword"] = newPassword
	payload["SessionAccountPassword"] = sessionAccountPassword
	return m.Post(m.changePasswordTarget, payload)
}

// ClearSecretKey shall clear the secret key for RFC6238-defined Time-based
// One-Time Password (TOTP) multi-factor authentication for this account.
func (m *ManagerAccount) ClearSecretKey() error {
	payload := make(map[string]any)
	return m.Post(m.clearSecretKeyTarget, payload)
}

// GenerateSecretKey shall randomly generate a new secret key for RFC6238-defined
...
// VerifyTimeBasedOneTimePassword shall verify a user-provided RFC6238-defined Time-based One-Time
// Password (TOTP).
...
// status code.
func (m *ManagerAccount) VerifyTimeBasedOneTimePassword(timeBasedOneTimePassword string) error {
	payload := make(map[string]any)
	payload["TimeBasedOneTimePassword"] = timeBasedOneTimePassword
	return m.Post(m.verifyTimeBasedOneTimePasswordTarget, payload)
}

// Role gets the Role linked resource.
func (m *ManagerAccount) Role(client common.Client) (*Role, error) {
	if m.role == "" {
		return nil, nil
	}
	return common.GetObject[Role](client, m.role)
}

// Certificates gets the Certificates collection.
func (m *ManagerAccount) Certificates(client common.Client) ([]*Certificate, error) {
	if m.certificates == "" {
		return nil, nil
	}
...
func (m *ManagerAccount) Keys(client common.Client) ([]*Key, error) {
	if m.keys == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[Key](client, m.keys)
}

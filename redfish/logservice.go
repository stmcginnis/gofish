...
//
// 2025.1 - #LogService.v1_8_1.LogService
...
func (l *LogService) UnmarshalJSON(b []byte) error {
	...
	type lActions struct {
		...
		PushDiagnosticData    common.ActionTarget `json:"#LogService.PushDiagnosticData"`
	}
	var tmp struct {
		temp
		Actions lActions
		Entries common.Link `json:"entries"`
	}
...
// ClearLog shall delete all entries found in the 'LogEntryCollection'
...
func (l *LogService) ClearLog(logEntriesETag string) error {
	payload := make(map[string]any)
	payload["LogEntriesETag"] = logEntriesETag
	return l.Post(l.clearLogTarget, payload)
}
...
// CollectDiagnosticData shall collect the diagnostic data for the given type. Upon
...
// diagnosticDataType - This parameter shall contain the type of diagnostic
// data to collect.
// oEMDiagnosticDataType - This parameter shall contain the OEM-defined type of
// diagnostic data to collect. This parameter shall be required if
// 'DiagnosticDataType' is 'OEM'.
// password - This parameter shall contain the password to access the URI
// specified by the 'TargetURI' parameter.
// targetDevice - This parameter shall contain a link to the resource that
// represents the device to collect diagnostic data. This parameter shall be
// required if 'DiagnosticDataType' contains 'Device'.
// targetURI - This parameter shall contain the URI to access when sending the
// diagnostic data. If this parameter is not provided by the client, the
// service shall not send the diagnostic data.
...
// specified by the 'TargetURI' parameter.
func (l *LogService) CollectDiagnosticData(parameters *CollectDiagnosticDataParameters) (string, *TaskMonitorInfo, error) {
	...
	resp, taskInfo, err := PostWithTask(l.GetClient(),
		l.collectDiagnosticDataTarget, parameters, l.Headers(), false)
...
// PushDiagnosticData shall send an existing diagnostic data to a target URI.
// additionalDataURI - This parameter shall contain the URI of the diagnostic
...
// userName - This parameter shall contain the username to access the URI
// specified by the 'TargetURI' parameter.
func (l *LogService) PushDiagnosticData(additionalDataURI string, password string, targetURI string, transferProtocol LogServiceTransferProtocolType, userName string) error {
	payload := make(map[string]any)
	payload["AdditionalDataURI"] = additionalDataURI
	payload["Password"] = password
	payload["TargetURI"] = targetURI
	payload["TransferProtocol"] = transferProtocol
	payload["UserName"] = userName
	return l.Post(l.pushDiagnosticDataTarget, payload)
}

// Entries gets the Entries collection.
func (l *LogService) Entries(client common.Client) ([]*common.LogEntry, error) {
	if l.entries == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[common.LogEntry](client, l.entries)
}
...
func (l *LogService) CollectDiagnosticDataActionInfo() (*ActionInfo, error) {
	...
	return common.GetObject[ActionInfo](l.GetClient(), l.collectDiagnosticInfoTarget)
}

...
func ListReferencedNetworkAdapterMetrics(c common.Client, link string) ([]*NetworkAdapterMetrics, error) { //nolint:dupl
	if link == "" {
		return nil, nil
	}
	...
	// Save unordered results into link-to-NetworkAdapterMetrics helper map.
	unorderedResults := map[string]*NetworkAdapterMetrics{}
	for r := range ch {
		if r.Error != nil {
	...
	results := make([]*NetworkAdapterMetrics, len(links))

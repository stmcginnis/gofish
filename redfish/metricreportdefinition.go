...
type MetricReportDefinition struct {
	...
	Wildcards []MetricReportDefinitionWildcard
...
func (m *MetricReportDefinition) UnmarshalJSON(b []byte) error {
	...
	type mLinks struct {
		Triggers common.Links `json:"Triggers"`
	}
	var tmp struct {
		temp
		Links        mLinks
		MetricReport common.Link `json:"metricReport"`
	}
...
func (m *MetricReportDefinition) Triggers(client common.Client) ([]*Triggers, error) {
	return common.GetObjects[Triggers](client, m.triggers)
}

// MetricReport gets the MetricReport linked resource.
func (m *MetricReportDefinition) MetricReport(client common.Client) (*MetricReport, error) {
	if m.metricReport == "" {
		return nil, nil
	}
	return common.GetObject[MetricReport](client, m.metricReport)
}
...
// MetricReportDefinitionWildcard shall contain a wildcard and its substitution values.
type MetricReportDefinitionWildcard struct {
	// Keys shall contain the list of values to substitute for the wildcard.
	//
	// Deprecated: v1.1.0

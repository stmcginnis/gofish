...
func ListReferencedCompositionServices(c common.Client, link string) ([]*CompositionService, error) { //nolint:dupl
	...
	// Save unordered results into link-to-CompositionService helper map.
	unorderedResults := map[string]*CompositionService{}
	for r := range ch {
		if r.Error != nil {
	...
	results := make([]*CompositionService, len(links))

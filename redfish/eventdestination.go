...
//
// 2024.3 - #EventDestination.v1_15_1.EventDestination
...
func (e *EventDestination) UnmarshalJSON(b []byte) error {
	...
	type eActions struct {
		ResumeSubscription  common.ActionTarget `json:"#EventDestination.ResumeSubscription"`
		SuspendSubscription common.ActionTarget `json:"#EventDestination.SuspendSubscription"`
	}
	var tmp struct {
		temp
		Actions            eActions
...
// ResumeSubscription shall resume a suspended event subscription, which affects the
...
func (e *EventDestination) ResumeSubscription(deliverBufferedEventDuration string) error {
	payload := make(map[string]any)
	payload["DeliverBufferedEventDuration"] = deliverBufferedEventDuration
	return e.Post(e.resumeSubscriptionTarget, payload)
}

// SuspendSubscription shall suspend an event subscription. No events shall be sent to
...
func (e *EventDestination) SuspendSubscription() error {
	payload := make(map[string]any)
	return e.Post(e.suspendSubscriptionTarget, payload)
}

// Certificates gets the Certificates collection.
func (e *EventDestination) Certificates(client common.Client) ([]*Certificate, error) {
	if e.certificates == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[Certificate](client, e.certificates)
}

// ClientCertificates gets the ClientCertificates collection.
func (e *EventDestination) ClientCertificates(client common.Client) ([]*Certificate, error) {
	if e.clientCertificates == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[Certificate](client, e.clientCertificates)
}
...
type SNMPSettings struct {
	...
	// TrapCommunity shall contain the SNMP trap community string. The value shall
...
type subscriptionPayload struct {
	...
	EventTypes          []common.EventType       `json:"EventTypes,omitempty"`

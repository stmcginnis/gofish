...
func (c *Certificate) UnmarshalJSON(b []byte) error {
	...
	type cActions struct {
		ForceAutomaticRenew common.ActionTarget `json:"#Certificate.ForceAutomaticRenew"`
		Rekey               common.ActionTarget `json:"#Certificate.Rekey"`
		Renew               common.ActionTarget `json:"#Certificate.Renew"`
	}
	type cLinks struct {
		Issuer   common.Link  `json:"Issuer"`
		Subjects common.Links `json:"Subjects"`
	}
	var tmp struct {
		temp
		Actions cActions
...
// ForceAutomaticRenew shall force an automatic renewal of the certificate, if this
...
func (c *Certificate) ForceAutomaticRenew() error {
	payload := make(map[string]any)
	return c.Post(c.forceAutomaticRenewTarget, payload)
}

// Rekey shall use the certificate data to generate a new key-pair for a
...
// table, formerly the 'TPM_ALG_ID Constants' table, within the 'Trusted
// Computing Group Algorithm Registry'.
func (c *Certificate) Rekey(challengePassword string, keyBitLength int, keyCurveID string, keyPairAlgorithm string) (*RekeyResponse, error) {
	payload := make(map[string]any)
	...
	resp, err := c.PostWithResponse(c.rekeyTarget, payload)
	if err != nil {
		return nil, err
	}
...
// Renew shall generate a certificate signing request using the existing
...
// CertificateIssuer gets the Issuer linked resource.
func (c *Certificate) CertificateIssuer(client common.Client) (*Certificate, error) {
	if c.issuer == "" {
		return nil, nil
	}
	return common.GetObject[Certificate](client, c.issuer)
}

// Subjects gets the Subjects linked resources.
func (c *Certificate) Subjects(client common.Client) ([]*Certificate, error) {
	return common.GetObjects[Certificate](client, c.subjects)
}

// Identifier shall contain the properties that identifies the issuer or subject
// of a certificate.
...
func (r *RekeyResponse) Certificate(client common.Client) (*Certificate, error) {
	if r.certificate == "" {
		return nil, nil
	}
	return common.GetObject[Certificate](client, r.certificate)
}
...
// Certificate gets the Certificate linked resource.
func (r *RenewResponse) Certificate(client common.Client) (*Certificate, error) {
	if r.certificate == "" {
		return nil, nil
	}
	return common.GetObject[Certificate](client, r.certificate)

...
func (s *SoftwareInventory) UnmarshalJSON(b []byte) error {
	...
	type sActions struct {
		Activate common.ActionTarget `json:"#SoftwareInventory.Activate"`
	}
	type sLinks struct {
		ActiveTargets common.Links `json:"ActiveTargets"`
		StagedTargets common.Links `json:"StagedTargets"`
	}
	var tmp struct {
		temp
		Actions     sActions
...
// Activate shall activate this software inventory instance.
...
func (s *SoftwareInventory) Activate(targets []string) error {
	payload := make(map[string]any)
	payload["Targets"] = targets
	return s.Post(s.activateTarget, payload)
}

// ActiveTargets gets the ActiveTargets linked resources.
func (s *SoftwareInventory) ActiveTargets(client common.Client) ([]*common.Entity, error) {
	return common.GetObjects[common.Entity](client, s.activeTargets)
}

// StagedTargets gets the StagedTargets linked resources.
func (s *SoftwareInventory) StagedTargets(client common.Client) ([]*common.Entity, error) {
	return common.GetObjects[common.Entity](client, s.stagedTargets)
}

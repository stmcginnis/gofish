...
const (
	...
	UnknownPowerSupplyType PowerSupplyType = "Unknown" // From power
)

// PowerSupply shall represent a power supply unit for a Redfish implementation.
...
type PowerSupply struct {
	...
	// the values shown in the 'NominalVoltageType' property in the 'InputRanges'
...
func (p *PowerSupply) UnmarshalJSON(b []byte) error {
	...
	type pLinks struct {
		Outlet          common.Link  `json:"Outlet"`
		PowerOutlets    common.Links `json:"PowerOutlets"`
		PoweringChassis common.Links `json:"PoweringChassis"`
	}
	var tmp struct {
		temp
		Actions      pActions
...
func GetPowerSupply(c common.Client, uri string) (*PowerSupply, error) {
	return common.GetObject[PowerSupply](c, uri)
}
...
// Reset shall reset a power supply. A 'GracefulRestart' 'ResetType'
...
// 'GracefulRestart'.
func (p *PowerSupply) Reset(resetType common.ResetType) error {
...
func (p *PowerSupply) Outlet(client common.Client) (*Outlet, error) {
	if p.outlet == "" {
		return nil, nil
	}
	return common.GetObject[Outlet](client, p.outlet)
}

// PowerOutlets gets the PowerOutlets linked resources.
func (p *PowerSupply) PowerOutlets(client common.Client) ([]*Outlet, error) {
	return common.GetObjects[Outlet](client, p.powerOutlets)
}

// PoweringChassis gets the PoweringChassis linked resources.
func (p *PowerSupply) PoweringChassis(client common.Client) ([]*Chassis, error) {
	return common.GetObjects[Chassis](client, p.poweringChassis)
}

// Assembly gets the Assembly linked resource.
func (p *PowerSupply) Assembly(client common.Client) (*Assembly, error) {
	if p.assembly == "" {
		return nil, nil
	}
...
func (p *PowerSupply) Certificates(client common.Client) ([]*Certificate, error) {
	if p.certificates == "" {
		return nil, nil
	}
...
func (p *PowerSupply) Metrics(client common.Client) (*PowerSupplyMetrics, error) {
	if p.metrics == "" {
		return nil, nil
	}
	return common.GetObject[PowerSupplyMetrics](client, p.metrics)
}
...
// PowerSupplyInputRange shall describe an input range that the associated power supply can
// utilize.

...
func (j *JobDocument) UnmarshalJSON(b []byte) error {
	...
	type jActions struct {
		SubmitJob common.ActionTarget `json:"#JobDocument.SubmitJob"`
	}
	type jLinks struct {
		SupportedExecutors common.Links `json:"SupportedExecutors"`
	}
	var tmp struct {
		temp
		Actions jActions
...
func (j *JobDocument) Update() error {
...
func GetJobDocument(c common.Client, uri string) (*JobDocument, error) {
	return common.GetObject[JobDocument](c, uri)
}

// ListReferencedJobDocuments gets the collection of JobDocument from
// a provided reference.
func ListReferencedJobDocuments(c common.Client, link string) ([]*JobDocument, error) {
	return common.GetCollectionObjects[JobDocument](c, link)
}

// SubmitJob shall create a new 'Job' resource based on the contents of this
// job document and additional parameters.
// hidePayload - This parameter shall indicate whether the contents of the
// parameters should be hidden from view after the job has been created. If
// 'true', responses shall not return the 'Parameters' property. If 'false',
// responses shall return the 'Parameters' property. If this parameter is not
// present when the job is created, the default is 'false'.
// jobCreator - This parameter shall contain a link to a resource of type 'Job'
// that represents the job that is submitting this job.
// parameters - This parameter shall contain the list of parameters for the new
// job that are specific to this job document. Services shall reject requests
// containing parameters that do not meet the requirements specified by the
// 'ParameterMetadata' property.
// preferredExecutors - This parameter shall contain an array of links to
// resources of type 'JobExecutor' that represent the preferred executors to
// run this job.
// startTime - This parameter shall contain the date and time when the job is
// scheduled to start.
func (j *JobDocument) SubmitJob(hidePayload bool, jobCreator string, parameters []any, preferredExecutors []string, startTime string) error {
	payload := make(map[string]any)
	payload["HidePayload"] = hidePayload
	payload["JobCreator"] = jobCreator
	payload["Parameters"] = parameters
	payload["PreferredExecutors"] = preferredExecutors
	payload["StartTime"] = startTime
	return j.Post(j.submitJobTarget, payload)
}

// SupportedExecutors gets the SupportedExecutors linked resources.
func (j *JobDocument) SupportedExecutors(client common.Client) ([]*JobExecutor, error) {
	return common.GetObjects[JobExecutor](client, j.supportedExecutors)
}

...
func (j *Job) UnmarshalJSON(b []byte) error {
	...
	type jActions struct {
		Cancel     common.ActionTarget `json:"#Job.Cancel"`
		ForceStart common.ActionTarget `json:"#Job.ForceStart"`
		Invalidate common.ActionTarget `json:"#Job.Invalidate"`
		Resubmit   common.ActionTarget `json:"#Job.Resubmit"`
		Resume     common.ActionTarget `json:"#Job.Resume"`
		Suspend    common.ActionTarget `json:"#Job.Suspend"`
		Validate   common.ActionTarget `json:"#Job.Validate"`
	}
	type jLinks struct {
		CreatedResources   common.Links `json:"CreatedResources"`
		Executor           common.Link  `json:"Executor"`
		JobDocument        common.Link  `json:"JobDocument"`
		ParentJob          common.Link  `json:"ParentJob"`
		PreferredExecutors common.Links `json:"PreferredExecutors"`
		SubsidiaryJobs     common.Links `json:"SubsidiaryJobs"`
		ValidatedExecutors common.Links `json:"ValidatedExecutors"`
	}
	var tmp struct {
		temp
		Actions jActions
		Links   jLinks
		Steps   common.Link `json:"steps"`
	}
...
// Cancel shall cancel the job if it is currently in the 'Running',
// 'Invalid', or 'Suspended' states. The job shall transition to 'Cancelled' if
// the action is successful.
func (j *Job) Cancel() error {
	payload := make(map[string]any)
	return j.Post(j.cancelTarget, payload)
}

// ForceStart shall force the job to start running if it is in the 'Pending'
// state. The job shall transition to 'Running' if the action is successful.
func (j *Job) ForceStart() error {
	payload := make(map[string]any)
	return j.Post(j.forceStartTarget, payload)
}

// Invalidate shall invalidate the job if it is in the 'Pending' state. The
// job shall transition to 'Invalid' if the action is successful.
func (j *Job) Invalidate() error {
	payload := make(map[string]any)
	return j.Post(j.invalidateTarget, payload)
}

// Resubmit shall resubmit a job to the job service. The new job shall be
...
func (j *Job) Resubmit(startTime string) error {
	payload := make(map[string]any)
	payload["StartTime"] = startTime
	return j.Post(j.resubmitTarget, payload)
}

// Resume shall resume the job if it is in the 'Suspended' state. The job
// shall transition to 'Running' if the action is successful.
func (j *Job) Resume() error {
	payload := make(map[string]any)
	return j.Post(j.resumeTarget, payload)
}

// Suspend shall suspend the job if it is in the 'Running' state. The job
// shall transition to 'Suspended' if the action is successful.
func (j *Job) Suspend() error {
	payload := make(map[string]any)
	return j.Post(j.suspendTarget, payload)
}

// Validate shall request the validation the job if it is in the 'New'
...
func (j *Job) Validate() error {
	payload := make(map[string]any)
	return j.Post(j.validateTarget, payload)
}

// CreatedResources gets the CreatedResources linked resources.
func (j *Job) CreatedResources(client common.Client) ([]*common.Entity, error) {
	return common.GetObjects[common.Entity](client, j.createdResources)
}

// Executor gets the Executor linked resource.
func (j *Job) Executor(client common.Client) (*JobExecutor, error) {
	if j.executor == "" {
		return nil, nil
	}
	return common.GetObject[JobExecutor](client, j.executor)
}

// JobDocument gets the JobDocument linked resource.
func (j *Job) JobDocument(client common.Client) (*JobDocument, error) {
	if j.jobDocument == "" {
		return nil, nil
	}
	return common.GetObject[JobDocument](client, j.jobDocument)
}

// ParentJob gets the ParentJob linked resource.
func (j *Job) ParentJob(client common.Client) (*Job, error) {
	if j.parentJob == "" {
		return nil, nil
	}
	return common.GetObject[Job](client, j.parentJob)
}

// PreferredExecutors gets the PreferredExecutors linked resources.
func (j *Job) PreferredExecutors(client common.Client) ([]*JobExecutor, error) {
	return common.GetObjects[JobExecutor](client, j.preferredExecutors)
}

// SubsidiaryJobs gets the SubsidiaryJobs linked resources.
func (j *Job) SubsidiaryJobs(client common.Client) ([]*Job, error) {
	return common.GetObjects[Job](client, j.subsidiaryJobs)
}

// ValidatedExecutors gets the ValidatedExecutors linked resources.
func (j *Job) ValidatedExecutors(client common.Client) ([]*JobExecutor, error) {
	return common.GetObjects[JobExecutor](client, j.validatedExecutors)
}

// Steps gets the Steps collection.
func (j *Job) Steps(client common.Client) ([]*Job, error) {
	if j.steps == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[Job](client, j.steps)
}
...
// Parameters shall contain the parameters to use when running this job. Each
// property-value pair specifies a parameter and its value as specified when the
// job was submitted.

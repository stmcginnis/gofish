...
func TestServerThermalParsing(t *testing.T) {
	...
	// Проверка основных полей Thermal
	...
	if len(thermal.Temperatures) != 51 {
		...
		if temp.Status.Health != common.OKHealth {
			t.Error("Expected temp health to be OK")
		}

		// Проверка датчика с null значениями
...
func TestServerFanOemParsing(t *testing.T) {
	...
	// Проверка OEM данных вентилятора
	fan := thermal.Fans[0]
	if len(fan.OEM) == 0 {
		t.Fatal("Expected OEM data in fan")
	}

	// Декодируем OEM структуру
	var oem struct {
		Temp struct {
...
func TestServerTemperatureThresholds(t *testing.T) {
	...
	// Проверка датчика с пороговыми значениями

...
func (ba *Battery) UnmarshalJSON(b []byte) error {
	...
	type baActions struct {
		Calibrate common.ActionTarget `json:"#Battery.Calibrate"`
		Reset     common.ActionTarget `json:"#Battery.Reset"`
		SelfTest  common.ActionTarget `json:"#Battery.SelfTest"`
	}
	type baLinks struct {
...
// Calibrate shall perform a self-calibration, or learn cycle, of the
// battery.
func (ba *Battery) Calibrate() error {
	payload := make(map[string]any)
	return ba.Post(ba.calibrateTarget, payload)
}

// Reset shall reset the battery.
...
// 'GracefulRestart'.
func (ba *Battery) Reset(resetType common.ResetType) error {
	payload := make(map[string]any)
	payload["ResetType"] = resetType
	return ba.Post(ba.resetTarget, payload)
}

// SelfTest shall perform a self-test of the battery.
func (ba *Battery) SelfTest() error {
	payload := make(map[string]any)
	return ba.Post(ba.selfTestTarget, payload)
}

// Memory gets the Memory linked resources.
func (ba *Battery) Memory(client common.Client) ([]*Memory, error) {
	return common.GetObjects[Memory](client, ba.memory)
}

// StorageControllers gets the StorageControllers linked resources.
func (ba *Battery) StorageControllers(client common.Client) ([]*StorageController, error) {
	return common.GetObjects[StorageController](client, ba.storageControllers)
}

// Assembly gets the Assembly linked resource.
func (ba *Battery) Assembly(client common.Client) (*Assembly, error) {
	if ba.assembly == "" {
		return nil, nil
	}
	return common.GetObject[Assembly](client, ba.assembly)
}

// Metrics gets the Metrics linked resource.
func (ba *Battery) Metrics(client common.Client) (*BatteryMetrics, error) {
	if ba.metrics == "" {
		return nil, nil
	}
	return common.GetObject[BatteryMetrics](client, ba.metrics)
}

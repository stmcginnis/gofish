...
func (f *Fan) UnmarshalJSON(b []byte) error {
	type temp Fan
	type fLinks struct {
		CoolingChassis common.Links `json:"CoolingChassis"`
	}
	var tmp struct {
		temp
		Links    fLinks
...
func (f *Fan) CoolingChassis(client common.Client) ([]*Chassis, error) {
	return common.GetObjects[Chassis](client, f.coolingChassis)
}

// Assembly gets the Assembly linked resource.
func (f *Fan) Assembly(client common.Client) (*Assembly, error) {
	if f.assembly == "" {
		return nil, nil
	}

...
func (m *MemoryChunk) UnmarshalJSON(b []byte) error {
	...
	var tmp struct {
		temp
		ChunkLink common.Link `json:"chunkLink"`
	}
...
func (m *MemoryChunk) ChunkLink(client common.Client) (*MemoryChunks, error) {
	if m.chunkLink == "" {
		return nil, nil
	}
	return common.GetObject[MemoryChunks](client, m.chunkLink)
}

...
func postObjectWithTask[T any,
	...
	entity, err := common.DecodeGenericEntity[T, PT](c, resp)
	return entity, nil, err
}

func ParseTaskMonitorInfo(c common.Client, resp *http.Response) *TaskMonitorInfo {
	...
	retryAfter, err := common.ParseRetryAfter(resp.Header.Get("Retry-After"))
	if err == nil {
...
func WaitForTaskMonitor(ctx context.Context, c common.Client, defaultPollRate time.Duration, taskMonitor *TaskMonitorInfo, taskChan chan<- *Task) (*http.Response, error) {
	...
	for {
		...
		if resp.StatusCode == http.StatusAccepted {
			...
			if retryAfter, err := common.ParseRetryAfter(resp.Header.Get("Retry-After")); err == nil {

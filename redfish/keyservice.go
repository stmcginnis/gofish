...
func (k *KeyService) UnmarshalJSON(b []byte) error {
	...
	var tmp struct {
		temp
		NVMeoFKeyPolicies common.Link `json:"nVMeoFKeyPolicies"`
		NVMeoFSecrets     common.Link `json:"nVMeoFSecrets"`
	}
...
func (k *KeyService) NVMeoFKeyPolicies(client common.Client) ([]*KeyPolicy, error) {
	if k.nVMeoFKeyPolicies == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[KeyPolicy](client, k.nVMeoFKeyPolicies)
}

// NVMeoFSecrets gets the NVMeoFSecrets collection.
func (k *KeyService) NVMeoFSecrets(client common.Client) ([]*Key, error) {
	if k.nVMeoFSecrets == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[Key](client, k.nVMeoFSecrets)
}

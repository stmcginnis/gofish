...
type SensorImplementationType string

const (
	// PhysicalSensorSensorImplementationType The reading is acquired from a physical
	// sensor.
	PhysicalSensorSensorImplementationType SensorImplementationType = "PhysicalSensor"
	// SynthesizedSensorImplementationType The reading is obtained by applying a
	// calculation on one or more properties or multiple sensors. The calculation
	// is not provided.
	SynthesizedSensorImplementationType SensorImplementationType = "Synthesized"
	// ReportedSensorImplementationType The reading is obtained from software or a
	// device.
	ReportedSensorImplementationType SensorImplementationType = "Reported"
)
...
type Sensor struct {
	...
	//
	// Version added: v1.1.0
	Implementation SensorImplementationType
	...
	//
	// Version added: v1.4.0
	SensorGroup common.RedundantGroup
...
func (s *Sensor) ResetMetrics() error {
	payload := make(map[string]any)
	return s.Post(s.resetMetricsTarget, payload)
}
...
func (s *Sensor) ResetToDefaults() error {
	payload := make(map[string]any)
	return s.Post(s.resetToDefaultsTarget, payload)
}

// AssociatedControls gets the AssociatedControls linked resources.
func (s *Sensor) AssociatedControls(client common.Client) ([]*Control, error) {
	return common.GetObjects[Control](client, s.associatedControls)
}
...
// SensorThresholds shall contain the set of thresholds that derive a sensor's health
// and operational range.

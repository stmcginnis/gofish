...
const (
	...
	// StartingLinkStatus This link on this interface is starting. A physical link
	...
	DownLinkStatus LinkStatus = "Down" // From networkport
	// UpLinkStatus The port is enabled and link is good (up).
	UpLinkStatus LinkStatus = "Up" // From networkport
)
...
const (
	...
	// SFP28SFPType The SFP conforms to the SFF Specification for SFP+ and IEEE
...
func (p *Port) UnmarshalJSON(b []byte) error {
	...
	type pActions struct {
		Reset    common.ActionTarget `json:"#Port.Reset"`
		ResetPPB common.ActionTarget `json:"#Port.ResetPPB"`
	}
	type pLinks struct {
		AssociatedEndpoints  common.Links `json:"AssociatedEndpoints"`
		Cables               common.Links `json:"Cables"`
		ConnectedPorts       common.Links `json:"ConnectedPorts"`
		ConnectedSwitchPorts common.Links `json:"ConnectedSwitchPorts"`
		ConnectedSwitches    common.Links `json:"ConnectedSwitches"`
...
func (p *Port) Reset(resetType common.ResetType) error {
	payload := make(map[string]any)
	payload["ResetType"] = resetType
	return p.Post(p.resetTarget, payload)
}

// ResetPPB shall reset the PCI-to-PCI bridge (PPB) for this port.
func (p *Port) ResetPPB() error {
	payload := make(map[string]any)
	return p.Post(p.resetPPBTarget, payload)
}

// AssociatedEndpoints gets the AssociatedEndpoints linked resources.
func (p *Port) AssociatedEndpoints(client common.Client) ([]*Endpoint, error) {
	return common.GetObjects[Endpoint](client, p.associatedEndpoints)
}

// Cables gets the Cables linked resources.
func (p *Port) Cables(client common.Client) ([]*Cable, error) {
	return common.GetObjects[Cable](client, p.cables)
}

// ConnectedPorts gets the ConnectedPorts linked resources.
func (p *Port) ConnectedPorts(client common.Client) ([]*Port, error) {
	return common.GetObjects[Port](client, p.connectedPorts)
}

// ConnectedSwitchPorts gets the ConnectedSwitchPorts linked resources.
func (p *Port) ConnectedSwitchPorts(client common.Client) ([]*Port, error) {
	return common.GetObjects[Port](client, p.connectedSwitchPorts)
}

// ConnectedSwitches gets the ConnectedSwitches linked resources.
func (p *Port) ConnectedSwitches(client common.Client) ([]*Switch, error) {
	return common.GetObjects[Switch](client, p.connectedSwitches)
}

// EthernetInterfaces gets the EthernetInterfaces linked resources.
func (p *Port) EthernetInterfaces(client common.Client) ([]*EthernetInterface, error) {
...
func (p *Port) EnvironmentMetrics(client common.Client) (*EnvironmentMetrics, error) {
...
func (p *Port) Metrics(client common.Client) (*PortMetrics, error) {
	if p.metrics == "" {
		return nil, nil
	}
	return common.GetObject[PortMetrics](client, p.metrics)
}

// PortCXL shall contain CXL-specific properties for a port.
type PortCXL struct {
...
type EthernetProperties struct {
	// AssociatedMACAddresses shall contain an array of configured MAC addresses
	// that are associated with this network port, including the programmed address
...
func (f *FunctionMaxBandwidth) NetworkDeviceFunction(client common.Client) (*NetworkDeviceFunction, error) {
	if f.networkDeviceFunction == "" {
		return nil, nil
	}
	return common.GetObject[NetworkDeviceFunction](client, f.networkDeviceFunction)
}
...
// NetworkDeviceFunction gets the NetworkDeviceFunction linked resource.
func (f *FunctionMinBandwidth) NetworkDeviceFunction(client common.Client) (*NetworkDeviceFunction, error) {
	if f.networkDeviceFunction == "" {
		return nil, nil
	}
	return common.GetObject[NetworkDeviceFunction](client, f.networkDeviceFunction)
...
// UnmarshalJSON unmarshals a PortGenZ object from the raw JSON.
func (g *PortGenZ) UnmarshalJSON(b []byte) error {
	type temp PortGenZ
	var tmp struct {
		temp
		LPRT common.Link `json:"lPRT"`
		MPRT common.Link `json:"mPRT"`
		VCAT common.Link `json:"vCAT"`
	}
...
func (g *PortGenZ) LPRT(client common.Client) ([]*RouteEntry, error) {
	if g.lPRT == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[RouteEntry](client, g.lPRT)
}

// MPRT gets the MPRT collection.
func (g *PortGenZ) MPRT(client common.Client) ([]*RouteEntry, error) {
	if g.mPRT == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[RouteEntry](client, g.mPRT)
}

// VCAT gets the VCAT collection.
func (g *PortGenZ) VCAT(client common.Client) ([]*VCATEntry, error) {
	if g.vCAT == "" {
		return nil, nil
	}
	return common.GetCollectionObjects[VCATEntry](client, g.vCAT)
}
...
// LinkConfiguration shall contain properties of the link for which this port
// has been configured.
...
// SFP shall describe a small form-factor pluggable (SFP) device attached to a
// port.
type SFP struct {
	...
	// producing the SFP. This organization may be the entity from which the SFP is
	...
	// Version added: v1.4.0
	Status common.Status
	// SupportedSFPTypes shall contain an array of SFP device types supported by
	// this port.
	//
	// Version added: v1.4.0
